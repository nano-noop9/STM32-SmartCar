...
#define MPU6050							//当前使用的传感器为 MPU6050
#define MOTION_DRIVER_TARGET_MSP430		//复用官方 MSP430 驱动接口，移植到 STM32F1
...
//}
#define log_i 	printf	//打印调试信息
#define log_e  	printf	//打印调试信息
...
uint8_t mpu_dmp_init(void)
{
	...
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//使能需要的传感器
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL);//配置 FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样率
		if(res)return 3; 
		res=dmp_load_motion_driver_firmware();		//加载 DMP 固件
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置传感器方向
		if(res)return 5; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//使能 DMP 功能
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置 DMP 输出速率（最大不超过 200Hz）
		if(res)return 7;   
...
//获取 DMP 解算后的姿态数据（注意：本函数局部变量较多，栈占用偏大）
