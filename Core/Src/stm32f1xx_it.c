...
/* USER CODE BEGIN 1 */
//串口接收回调函数
extern volatile int8_t carmode;

static uint8_t RxPacket[100];
static uint16_t RxPacketLen = 0;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		uint8_t tmp = Usart3ReceiveData;
		// 快速路径：遥控命令 - 直接调用motorPidSetSpeed以保证<100ms响应时延
		if(tmp == 'A') motorPidSetSpeed(1,1);
		else if(tmp == 'B') motorPidSetSpeed(-1,-1);
		else if(tmp == 'C') motorPidSetSpeed(0,0);
		else if(tmp == 'D') motorPidSetSpeed(1,2);
		else if(tmp == 'E') motorPidSetSpeed(2,1);
		else if(tmp == 'F') motorSpeedUp();
		else if(tmp == 'G') motorSpeedCut();
    ...
    else if(tmp == 'J')  // 蓝牙：模式递增
    {
      carmode = (carmode % 5) + 1;  // 循环1→2→3→4→5→1
    }
    else if(tmp == 'K')  // 蓝牙：回到模式0（停止）
    {
      carmode = 0;
    }
		...
		else if(rxState == 1)
		{
			...
			{
				rxState = 0;
				Joystick_ParseData(RxPacket, RxPacketLen);
			}
			else
			{
				if(RxPacketLen < 99)
				{
					RxPacket[RxPacketLen++] = tmp;
				}
			}
		}
